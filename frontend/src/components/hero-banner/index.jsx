...
export default function HeroBanner({ ctaText = 'Explore Products' }) {
  ...
  /* Autoplay fallback on first user gesture in case browser policy pauses */
  useEffect(() => {
    const resumeIfPaused = () => {
      const v = videoRefs[current]?.current;
      if (v && v.paused) v.play().catch(() => {});
    };
    window.addEventListener('click', resumeIfPaused, { once: true, passive: true });
    window.addEventListener('touchstart', resumeIfPaused, { once: true, passive: true });
    window.addEventListener('scroll', resumeIfPaused, { once: true, passive: true });
    return () => {
      window.removeEventListener('click', resumeIfPaused);
      window.removeEventListener('touchstart', resumeIfPaused);
      window.removeEventListener('scroll', resumeIfPaused);
    };
  }, [current]);
