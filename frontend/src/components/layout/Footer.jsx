...
export default function Footer() {
  ...
  async function handleSubscribe(e) {
    ...
    try {
      ...
      setSubError(err.response?.data?.error || 'Could not subscribe. Please try again.')
    } finally {
      setSubLoading(false)
    }
  }
                  ...
                  {subscribed ? (
                    ...
                    <>
                      <form className="form-sub mb-16" onSubmit={handleSubscribe}>
                        ...
                        <button type="submit" className="btn-action" disabled={subLoading}>
                          <i className="icon icon-ArrowUpRight"></i>
                        </button>
                      </form>
                      {subError && <p style={{ color: '#FCA5A5', fontSize: 12, marginTop: -8, marginBottom: 8 }}>{subError}</p>}
                    </>
                  )}
