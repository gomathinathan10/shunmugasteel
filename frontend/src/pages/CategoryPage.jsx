...
const FALLBACK = {
  'flat-products': {
    ...
    products: [
      ...
      { id: 3, name: 'GP Sheets & Coils', slug: 'gp-sheets-coils', product_type: 'standard', brand: 'JSW', image_url: 'gpsheetcoilc6bd.jpg', base_price: 85000, stock_status: 'in_stock', sub_category: 'GP/GC Sheets', description: 'Galvanized Plain sheets with zinc coating.' },
      ...
      { id: 6, name: 'GP Slitted Coil', slug: 'gp-slitted-coils', product_type: 'custom', brand: 'JSW', image_url: 'gp-slit-coil-952cf2f.jpg', base_price: null, stock_status: 'in_stock', sub_category: 'Slitted Coils', description: 'Galvanized Plain slit coils cut to specified widths.' },
      { id: 7, name: 'CR Slitted Coil', slug: 'cr-slitted-coils', product_type: 'custom', brand: 'Evonith', image_url: '0-25mm-cold-rolled-coil-1000x1000cf88.jpg', base_price: null, stock_status: 'in_stock', sub_category: 'Slitted Coils', description: 'Cold Rolled slit coils for precision applications.' },
    ],
  },
  'roofing-products': {
    ...
    subCategories: ['All', 'Corrugated Sheets', 'Decking Sheets', 'Insulated Panels', 'Transparent Sheets', 'Structural'],
    products: [
      { id: 4, name: 'Galvanized Corrugated Sheets', slug: 'galvanized-corrugated-sheets', product_type: 'standard', brand: 'SAIL', image_url: 'galvanized-corrugated-sheets7d36.jpg', base_price: 80000, stock_status: 'in_stock', sub_category: 'Corrugated Sheets', description: 'Galvanized Corrugated Sheets for roofing.' },
      { id: 8, name: 'Galvanized Decking Sheets', slug: 'steel-decking-sheets', product_type: 'custom', brand: 'Evonith', image_url: 'decking-sheets-17e37.jpg', base_price: null, stock_status: 'in_stock', sub_category: 'Decking Sheets', description: 'Steel deck sheets for composite flooring.' },
...
// Category aliases
FALLBACK['flat-product'] = FALLBACK['flat-products']
FALLBACK['flat'] = FALLBACK['flat-products']
FALLBACK['roofing'] = FALLBACK['roofing-products']
FALLBACK['roofing-product'] = FALLBACK['roofing-products']
FALLBACK['accessory'] = FALLBACK['accessories']

function normalizeCategory(slug) {
  if (!slug) return 'flat-products'
  const s = slug.toLowerCase().trim()
  if (s === 'flat' || s === 'flat-product' || s === 'flat-products') return 'flat-products'
  if (s === 'roofing' || s === 'roofing-product' || s === 'roofing-products') return 'roofing-products'
  if (s === 'acc' || s === 'accessory' || s === 'accessories') return 'accessories'
  return s
}
...
export default function CategoryPage({ defaultCategory }) {
  const { categorySlug: rawSlug } = useParams()
  const categorySlug = normalizeCategory(rawSlug || defaultCategory || 'flat-products')
  const initialFb = FALLBACK[categorySlug] || FALLBACK['flat-products']

  const [category, setCategory] = useState(initialFb)
  const [products, setProducts] = useState(initialFb?.products || [])
  const [activeTab, setActiveTab] = useState('All')
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    const fb = FALLBACK[categorySlug] || FALLBACK['flat-products']
    setCategory(fb)
