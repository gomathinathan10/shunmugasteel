...
export default function QuoteBasketPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const zeroStandard = items.find((i) => !i.is_custom && Number(i.unit_price) === 0)
    if (zeroStandard) {
      setError(`"${zeroStandard.product?.name}" has no price set. Please select a valid variant before submitting.`)
      return
    }
    ...
    try {
      ...
      const quoteId = res.data?.quote_id || res.data?.data?.id || res.data?.id
      clearBasket()
  ...
  if (items.length === 0) {
    return (
      <div className=" max-w-3xl mx-auto px-4 py-20 text-center" style={{ marginTop: '70px' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-gray-200 rounded-xl p-5 sticky top-24">
            ...
            {items.some((i) => !i.is_custom && Number(i.unit_price) === 0) && (
              <div className="bg-amber-50 border border-amber-300 text-amber-800 text-xs rounded-lg px-3 py-2 mb-3 flex items-start gap-2">
                <span style={{ fontSize: 14, flexShrink: 0 }}>&#9888;</span>
                <span>One or more standard items have no price. Please select a valid variant.</span>
              </div>
            )}
            ...
            <button
              ...
              disabled={submitting}
              className="w-full py-3 rounded-lg font-semibold text-white transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              style={{ background: '#E67E22', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              onMouseOver={(e) => { if (!submitting) e.currentTarget.style.background = '#d35400' }}
              onMouseOut={(e) => { if (!submitting) e.currentTarget.style.background = '#E67E22' }}
            >
