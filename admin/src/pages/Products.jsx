...
export default function Products() {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([
      getProducts().then((r) => setProducts((Array.isArray(r.data) ? r.data : []).filter((p) => p.status === 'active'))),
      getCategories().then((r) => setCats((Array.isArray(r.data) ? r.data : []).filter((c) => c.status === 'active'))),
  ...
  const handleSave = async (e) => {
    ...
    if (['standard', 'both'].includes(form.product_type)) {
      const namedVariants = variants.filter((v) => v.variant_name?.trim())
      if (!namedVariants.length) return setError('Add at least one variant with a name and price.')
      const missingPrice = namedVariants.find((v) => !v.price_per_unit || parseFloat(v.price_per_unit) <= 0)
      if (missingPrice) return setError(`Variant "${missingPrice.variant_name}" is missing a price. Please add a price before saving.`)
    }

    if (['custom', 'both'].includes(form.product_type)) {
      const priceFields = ['price_per_kg', 'price_per_ton', 'price_per_meter', 'price_per_sqft', 'price_per_sheet']
      const hasPrice = priceFields.some((f) => rule[f] && parseFloat(rule[f]) > 0)
      if (!hasPrice) return setError('Please set at least one pricing rate (per kg, per ton, etc.) in the Pricing tab.')
    }
  ...
  const toggleStatus = async (prod) => {
    ...
    setProducts((prev) => newStatus === 'inactive'
                    ...
                    >
                      ...
                      <td style={S.tdActions}>
                        <div style={S.actionsWrap}>
                          ...
                          {isLive && (
                            <a
                              href={`${PUBLIC_URL}/product/${p.slug}`}
                              target="_blank"
                              rel="noreferrer"
