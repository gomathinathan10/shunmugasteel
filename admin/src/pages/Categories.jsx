...
export default function Categories() {
  ...
  const load = () => {
    setLoading(true)
    getCategories().then((r) => setCats((Array.isArray(r.data) ? r.data : []).filter((c) => c.status === 'active'))).catch(() => setCats([])).finally(() => setLoading(false))
  }
  useEffect(() => { load() }, [])
  ...
  const toggleStatus = async (cat) => {
    ...
    setCats((prev) => newStatus === 'inactive'
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              {form.image && (
                <img
                  src={form.image.startsWith('http') ? form.image : `${import.meta.env.VITE_API_URL || 'http://localhost/shunmugasteel/backend'}/${form.image}`}
                  alt=""
