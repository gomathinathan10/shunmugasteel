...
elseif ($segments[0] === 'contact') {
    ...
    match([$method, $segments[1] ?? '']) {
        ['POST', 'submit'] => $ctrl->submit(),
        default            => respond(404, 'Route not found'),
    };
}

// Newsletter subscribe — public
elseif ($segments[0] === 'newsletter') {
...
elseif ($segments[0] === 'admin') {
    ...
    // Sub-resource routes: /admin/products/:id/variants|pricing|images
        ...
        match([$sub]) {
            ...
            ['newsletter']  => (new NewsletterController())->getSubscribers(),
            ['contact']     => $id
                ? (new ContactController())->update((int)$id)
                : (new ContactController())->getEnquiries(),
