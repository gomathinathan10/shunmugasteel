...
class QuoteController {
    ...
    private function calculateCustomPrice(array $rule, array $specs, float $quantity): float {
        $unit     = $rule['primary_pricing_unit'];
        $thick_mm = (float)($specs['thickness'] ?? 0); // mm
        $width_mm = (float)($specs['width']     ?? 0); // mm
        $len_m    = (float)($specs['length']    ?? 0); // meters

        // Steel density: 7850 kg/m³
        $weight_kg = ($thick_mm / 1000) * ($width_mm / 1000) * $len_m * 7850;

        // Area in sq.ft
        $area_sqft = ($width_mm / 304.8) * ($len_m / 0.3048);

        return match($unit) {
            'kg'    => $weight_kg > 0 ? $weight_kg * (float)$rule['price_per_kg']   : (float)$rule['price_per_kg'],
            'ton'   => $weight_kg > 0 ? ($weight_kg / 1000) * (float)$rule['price_per_ton'] : (float)$rule['price_per_ton'],
            'meter' => $len_m    > 0 ? $len_m    * (float)$rule['price_per_meter'] : (float)$rule['price_per_meter'],
            'sqft'  => $area_sqft > 0 ? $area_sqft * (float)$rule['price_per_sqft'] : (float)$rule['price_per_sqft'],
